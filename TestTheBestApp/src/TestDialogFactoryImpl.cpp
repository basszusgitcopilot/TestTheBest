...
auto TestDialogFactoryImpl::createTestDialog(std::string propertiesFile) -> std::shared_ptr<KnowledgeTestDialog> {
    auto testDialog = std::make_shared<KnowledgeTestDialogImpl>();
    auto configuration = TestItemListFactory::readConfigurationFromFile(propertiesFile);
    auto listItems = m_testItemListFactory->createTestItemList(configuration);
