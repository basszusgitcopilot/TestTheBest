...
auto TestItemVectorCrossProduct::equals(const TestItem &other) const -> bool {
    auto otherItem = dynamic_cast<const TestItemVectorCrossProduct &>(other);
    return getRightAnswer() == otherItem.getRightAnswer() || getQuestion() == otherItem.getQuestion();
};

auto TestItemVectorCrossProduct::getQuestion() const -> std::string {
    return "Hektor möchte das Vektorprodukt (Kreuzprodukt) von zwei Vektoren bekommen: (" + std::to_string(a1) + "; " + std::to_string(a2) + "; " +
